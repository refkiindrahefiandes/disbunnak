<?php
class Pengaduan extends Frontnologin_controller {
    ...
    public function index($offset = 0) {
        ...
        if ($this->form_validation->run() == TRUE) {
            ...
            if ($this->save('pengaduan', $data_post)) {
                $this->session->set_flashdata('success', lang('success_save'));
                redirect(site_url('pengaduan'));
            }
        }

        $this->data['subview'] = 'frontend/' . $this->data['active_theme'] . '/' . 'pengaduan';
    	$this->load->view('frontend/' . $this->data['active_theme'] . '/_layout_main', $this->data);
    }

    public function save($table = 'pengaduan', $data = array())
    {
        $this->db->set($data);
        $this->db->insert($table);
        // printr($this->db->last_query());die;

        // Create no urut REGISTRASI
			$id_no =  sprintf("%05d", $this->db->insert_id());
			$this->db->set(array('noregistrasi' => 'P'.$id_no));
			$this->db->where('pengaduan_id ', $id_no);
    ...
    {
        if ($id) {
            $pesan = $this->front_m->get_balasan($id);
    ...
    public function laporan()
    {
        ...
        $this->data['subview'] = 'frontend/' . $this->data['active_theme'] . '/' . 'front/pengaduan_laporan';
    	$this->load->view('frontend/' . $this->data['active_theme'] . '/_layout_main', $this->data);
    }
    ...
    public function fetch_data()
    {
        ...
        $config = array(
            ...
            'aColumns'            => array('pengaduan_id', 'type', 'content',  'status', 'date_added')
        );
