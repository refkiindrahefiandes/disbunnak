<?php
class Perizinan_berkbli extends Admin_controller {

    public function __construct()
    {
        parent::__construct();
        $this->load->model('pelayanan/perizinan_berkbli_m');
        $this->load->model('pelayanan/bidang_perizinan_berkbli_m');
    }

    public function index()
    {
        if ($this->user_m->hasPermission('access', 'pelayanan/Perizinan_berkbli')) {
            $this->data['subview'] = 'admin/pelayanan/perizinan_berkbli/index';
        } else {
    ...
    {
        $config = array(
            'model_name'          => 'perizinan_berkbli_m',
            'sIndexColumn'        => 'id',
            'table_name'          => 'layanan_perizinan_berkbli',
        ...
        if (! empty($query['aaData'])) {
            foreach ($query['aaData'] as $value) {
                $rResult['aaData'][] = array (
                    '0' => $value[0],
                    '1' => $value[1],
                    '2' => $this->bidang_perizinan_berkbli_m->get($value[2], TRUE)['name'],
                    '3' => $value[3],
                    '4' => $value[4],
                    '5' => $value[5]
                );
    ...
    {
        $this->data['data_bidang']  = $this->bidang_perizinan_berkbli_m->get();

        $result = $this->perizinan_berkbli_m->get_by(array('id' => $id), TRUE);
        ...
        if ($result) {
            ...
            $this->data['data'] = $this->perizinan_berkbli_m->get_new();
        }
        ...
        if ($this->form_validation->run() == TRUE) {
            ...
            if ($this->perizinan_berkbli_m->save($data, $id)) {
                $this->session->set_flashdata('success', lang('success_update_data'));
                redirect('admin/pelayanan/perizinan_berkbli');
            }
        }

        // Load the view
        $this->data['subview'] = 'admin/pelayanan/perizinan_berkbli/edit';
        $this->load->view('admin/_layout_main', $this->data);
    }

    public function status($id, $status)
    {
        if ($this->user_m->hasPermission('modify', 'pelayanan/Perizinan_berkbli')) {
            if ($status == 'true') {
                if ($this->perizinan_berkbli_m->update_status($id, 1)) {
                    $json['success'] = lang('status_update_success');
                    $this->output->set_output(json_encode($json));
                }

            } elseif ($status == 'false') {
                if ($this->perizinan_berkbli_m->update_status($id, 0)) {
                    $json['success'] = lang('status_update_success');
                    $this->output->set_output(json_encode($json));
                }
            }
    ...
    {
        if ($id) {
            $this->perizinan_berkbli_m->delete($id);
    ...
    {
        if ($bidang_id != NULL) {
            $result = $this->pelayanan_m->get_data($bidang_id, 'bidang_perizinan_berkbli', 'bidang_id');
